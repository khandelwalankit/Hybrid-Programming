...
template<typename INT,typename VARY>
  ...
  { 
    VARY sum=0;
    int limitx=localx+slicex;
    int limity=localy+slicey;
    for(int i=localx;i<limitx;i++)
    {
      int n=0;
      for(int j=localy;j<limity;j++)
      {
        sum=0;
        int m=0;
        for(int k=localy;k<limity;k++)
        {
    ...
    for(int i=0;i<slicex;i++)
     { 
      for(int j=0;j<slicey;j++)
        ...
        {
          sum+=temp[i*slicey+k]*mat[k*slicey+j];
        }
        mult[i*slicey+j]=sum;
        }
     } 
...
template<typename INT, typename VARY>
  void matrix_subtract(INT localx,INT localy,INT slicex,INT slicey,INT ydim, VARY *A,VARY *mat)
  {
    ...
    for(int i=localx;i<limitx;i++)
    {
      ...
      for(int j=localy;j<limity;j++)
      {
        A[i*ydim+j]=A[i*ydim+j]-mat[r*slicey+c];
        c++;
      }
      r++;
    }
...
template<typename INT,typename VARY>
void lu_invert(INT slicex,INT slicey, INT ydim, VARY *A)  {
